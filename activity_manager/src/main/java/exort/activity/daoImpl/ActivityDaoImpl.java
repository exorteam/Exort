...
public class ActivityDaoImpl implements ActivityDao {
    ...
    public Response update(Activity activity){
        try{
            Query query = Query.query(Criteria.where("id").is(activity.getId()));
            mongoTemplate.upsert(query, new Update(), Activity.class);
    ...
    public Response selectActivities(Select select, int pagesize, int pagenum, int sortby){
       try{
           ...
           if(select.getCreateTime()!=null){
               query.addCriteria(Criteria.where("createTime").gte(select.getCreateTime().getStart()).and("createTime").lte(select.getCreateTime().getEnd()));
           }
           if(select.getSignupTime()!=null){
               query.addCriteria(Criteria.where("SignupTime").gte(select.getSignupTime().getStart()).and("signupTime").lte(select.getSignupTime().getEnd()));
           }
           if(select.getStartTime()!=null){
               query.addCriteria(Criteria.where("createTime").gte(select.getStartTime().getStart()).and("createTime").lte(select.getStartTime().getEnd()));
           }
    ...
    @Override
    public Response checkUserId(Long activityid, Long userId, int type){
        try{
            Activity activity = mongoTemplate.findById(activityid, Activity.class);
            if(activity!=null){
                boolean exist;
